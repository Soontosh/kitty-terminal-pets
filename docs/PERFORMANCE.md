...
- Renderers block in Linux `inotify` or macOS `kqueue` instead of waking five times per second.
...
The optimized service also runs at nice level 10 so real terminal work wins under contention. Linux additionally uses low systemd CPU/I/O weights and idle-class I/O scheduling; the macOS launch agent uses launchd's background process type and low-priority I/O policy.
...
Existing renderers react to configuration through `inotify` on Linux and `kqueue` on macOS; no restart is needed for ordinary config changes. Changing service installation itself still requires rerunning `install.sh`.
